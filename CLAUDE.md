...
- `--alpha` couples three otherwise-independent options: it forces `opts.codec === 'prores_ks'`, `opts.container === 'mov'`, and `opts.captureFormat === 'png'`, and rejects explicit user flags that conflict with any of them. The coupling lives in `resolveExportOpts` and runs *before* the codec/container compatibility check so the forced values participate in that validation. Don't move the check around without re-running `node cli.js export tests/alpha-test.html --alpha --dry-run` and the negative paths (`--alpha --codec libx264`, `--alpha --capture-format jpeg`).
