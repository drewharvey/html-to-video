...
- The single-file viewport convention (`<meta name="h2v-viewport" content="WxH">`) and bundle marker `viewport="WxH"` attribute. Format is `<width>x<height>` in CSS pixels, integers only — `parseBundleFrames` and `extractViewport` enforce this. Default when absent is `DEFAULT_VIEWPORT` (1280x720). Currently consumed only by `buildReviewAnimations` → `buildReviewHtml` to size each iframe correctly across mixed aspects. `h2v export` does not yet honor it (uses global `--width`/`--height`); the meta is forward-compatible for that future change. Don't rename or change the format without considering the downstream consumers.
