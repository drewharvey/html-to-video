...
- `h2v export` — converts HTML animations to video files via Puppeteer + ffmpeg. Default output is 4K 60fps **10-bit HEVC** MP4 (h265, `yuv420p10le`, `-tag:v hvc1`); 10-bit is what avoids the macroblock/gradient-banding artifacts 8-bit h264 produces on smooth content. Use `--codec libx264` for 8-bit h264 when maximum web/older-device compatibility matters. Use `--quality-preset max|high|standard|draft` to switch between archival, high-quality-distribution, default, and fast-iteration tiers; or override individual knobs with `--codec` (libx264|libx265|libvpx-vp9|prores_ks), `--container` (mp4|mov|webm), `--capture-format` (jpeg|png), `--capture-quality`, `--crf`, `--fps`, `--width`, `--height`, `--scale`, `--slowdown`. Explicit flags always override the preset.
...
- The `--quality-preset` table in `QUALITY_PRESETS`: `max | high | standard | draft`. Each preset bundles `captureFormat`, `captureQuality`, `codec`, `crf`. Codec-specific encoder choices (pix_fmt, x264/x265 `-preset`, `-tune`, prores profile) are derived from `opts.qualityPreset` *inside* `buildEncodeArgs` — the preset table itself stays codec-agnostic. `resolveExportOpts` applies preset values only to fields the user didn't explicitly pass, gated by the `*Explicit` booleans on `opts`. Default is `standard`; passing no `--quality-preset` flag is identical to `--quality-preset standard`. **Codecs per tier: `standard`→`libx265`, `high`→`libx265`, `draft`→`libx264`, `max`→`prores_ks`.** `standard`/`high` are libx265 specifically so the default and the quality tier get 10-bit (see the bit-depth invariant below); `draft` stays `libx264` because x265 is 2–4× slower to encode and draft is the throwaway speed tier (it knowingly keeps 8-bit artifacts).
- **Bit depth is keyed off the codec, not the tier, in `buildEncodeArgs`.** For `libx264`/`libx265` the chroma is tier-driven (`high`/`max`→`yuv444p`, else `yuv420p`), but the bit depth is `opts.codec === 'libx265' ? <chroma>+'10le' : <chroma>` — i.e. **HEVC always encodes 10-bit, h264 always 8-bit.** This is load-bearing: 10-bit is what clears the localized macroblock ("hammered") artifacts and gradient banding that 8-bit DCT produces on smooth/animated content (verified against a real 1080×1920 gradient clip; 8-bit h264 *and* 8-bit HEVC both showed it, 10-bit cleared it), at no size cost (10-bit was ~33% smaller than 8-bit on that clip). h264 is deliberately never auto-10-bit: 10-bit h264 is the High 10 profile, which has no hardware decode and is rejected by QuickTime/Safari — so an explicit `--codec libx264` must stay 8-bit. `-profile:v high444` is only emitted for the literal 8-bit `yuv444p` (h264 high/max); libx265 derives its profile from the pix_fmt. Don't reintroduce a tier-based bit-depth rule, and don't let `--codec libx264` produce a `*10le` pix_fmt.
- **libx265 capability probe + fallback (`ensureCodecOrFallback`, called after `ensureFfmpeg` in `main`, skipped under `--no-ffmpeg`/`--dry-run`).** The default codec is libx265-10bit, but minimal/LGPL ffmpeg builds may lack libx265 and a rare legacy libx265 may be 8-bit-only. `libx265TenBitAvailable()` probes `ffmpeg -encoders` for libx265 and `ffmpeg -h encoder=libx265` for `yuv420p10le`. When the resolved codec is `libx265` but the probe fails: an **explicit** `--codec libx265` (`codecExplicit`) **errors** (respect stated intent); a preset-derived libx265 (incl. the default) **falls back to `libx264`** with a warning that the artifact fix is lost. Fallback only flips `opts.codec` — the already-resolved container (mp4/mov) is valid for libx264 too, and `buildEncodeArgs` then emits 8-bit automatically. The warning wording mentions "compression artifacts (macroblocking / gradient banding)", not just banding (the macroblock artifacts are the worse regression). Don't move the probe before `ensureFfmpeg` (it shells out to ffmpeg) or into `resolveExportOpts` (which is non-shelling and runs in `--dry-run`).
