...
- `--alpha` couples three otherwise-independent options: it forces `opts.captureFormat === 'png'`, `opts.container === 'mov'`, and constrains `opts.codec` to one of `{png, prores_ks}` (default `png`). It also steps `opts.fps` down to `DEFAULTS.alphaFps` (30) when `--fps` wasn't passed explicitly — alpha codecs are far larger per-frame than h264 and 60fps doubles the file size for no compositing-quality gain. The coupling lives in `resolveExportOpts` and runs *before* the codec/container compatibility check so the forced values participate in that validation. Don't move the check around without re-running `node cli.js export tests/alpha-test.html --alpha --dry-run` and the negative paths (`--alpha --codec libx264`, `--alpha --capture-format jpeg`, `--alpha --codec libvpx-vp9`).
- `--alpha`'s default codec is `png` (PNG-in-MOV), not `prores_ks`. The reason is alpha-mode metadata: Chrome's `Page.captureScreenshot` produces **straight alpha** (verified by pixel sampling — `R=255 B=156 α=13` in glow regions), and the PNG codec spec mandates straight alpha at the codec level so every player interprets identically. ffmpeg's `prores_ks` muxer does not write the `kCMFormatDescriptionExtension_AlphaChannelMode` extension, so the resulting file is ambiguous — QuickTime/FCP guess straight (correct), CapCut and various web editors guess premultiplied (wrong; semi-transparent glows blow out into bright halos). Don't switch the default back to `prores_ks` without first solving the metadata-tagging problem or pre-multiplying with `-vf premultiply=inplace=1` before encode (which then breaks the QuickTime/FCP path — both can't be served from one file without explicit metadata). `prores_ks` remains opt-in via `--codec prores_ks` for users whose pipeline specifically expects it.
- The `png` case in `buildEncodeArgs` is intentionally minimal: `-c:v png` and `-pix_fmt rgba` when alpha is on, `-pix_fmt rgb24` otherwise. Both are bit-exact lossless. The PNG codec has no quality knobs — `--crf` and `--quality-preset` are silently ignored. Don't add a CRF arg to the case; PNG codec doesn't accept one.
