...
- **Virtual time = setVirtualTimePolicy + beginFrame, both required** — `Emulation.setVirtualTimePolicy` virtualizes only the *timer* clock (Date, setTimeout, etc.). The compositor (which runs CSS animations/transitions) has its own clock that ignores setVirtualTimePolicy. To virtualize CSS timing too, you also have to drive the compositor with `HeadlessExperimental.beginFrame` per output frame, passing `frameTimeTicks`. We learned this the hard way on frame-09: setVirtualTimePolicy alone left the SVG `stroke-dashoffset` transition on the wall clock. Don't drop beginFrame.
- **`headless: 'shell'`** — `HeadlessExperimental` is only exposed in `chrome-headless-shell` (old headless). `headless: true` (new headless) does not have it. The shell binary is auto-downloaded by `npm install puppeteer`. Don't switch back to new headless.
- **`--enable-begin-frame-control`** and **`--run-all-compositor-stages-before-draw`** launch flags are required. The first enables `beginFrame`; the second forces the compositor to commit all queued work before each screenshot. Don't drop either.
- **Per-target `enableBeginFrameControl`** — `--enable-begin-frame-control` only enables the *feature* at the browser level. Each Target also has to be created with `enableBeginFrameControl: true` via `Target.createTarget`, otherwise `HeadlessExperimental.beginFrame` will close the target on its first call ("TargetCloseError" mid-recording). Puppeteer's `browser.newPage()` doesn't pass this option, so we call `Target.createTarget` ourselves in `newPageWithBeginFrameControl`. Don't switch back to plain `browser.newPage()`.
